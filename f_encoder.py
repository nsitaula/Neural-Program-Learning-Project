""""This code is same from https://github.com/siddk/npi with addition.py
...
class F_Encoder():
    def __init__(self, hidden_dim=100, state_dim=128, batch_size=1):
        """
        Instantiate an  Core object, with the necessary hyperparameters.
        """
        self.hidden_dim, self.state_dim, self.bsz = hidden_dim, state_dim, batch_size
        self.env_dim = (CONFIG["ENVIRONMENT_ROW"] * CONFIG["ENVIRONMENT_COL"]) +1  # 4 * 10 = 40
        ...
        self.program_key = tflearn.variable(name='Program_Keys', shape=[CONFIG["PROGRAM_NUM"], CONFIG["PROGRAM_KEY_SIZE"]], initializer='truncated_normal')
        self.program_embedding = self.build_program_store()

    def build_encoder(self):
        """
        Build the Encoder Network (f_enc) taking the environment state (env_in)
         feeding through a Multilayer Perceptron, to generate the state encoding
        (s_t).
        ...
        """
        elu = tflearn.fully_connected(self.env_in, self.hidden_dim, activation='elu')
    ...
    def build_program_store(self):
        ...
        """
        embedding = tflearn.embedding(self.prg_in, CONFIG["PROGRAM_NUM"], CONFIG["PROGRAM_EMBEDDING_SIZE"], name="Program_Embedding")
        return embedding
